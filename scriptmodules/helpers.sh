...
#
# Core script functionality is based upon The ARES Project https://ares.org.uk Script Modules
#

## @file helpers.sh
## @brief ARES helpers library
## @copyright GPLv3
...
function getDepends() {
    ...
    iniConfig " = " '"' "$configdir/all/ares.cfg"
    iniGet "own_sdl2"
    ...
    for required in $@; do
        ...
        if [[ "$required" == "libpng12-dev" ]] && compareVersions "$__os_debian_ver" ge 9;  then
            required="libpng-dev"
            printMsgs "console" "ARES module references libpng12-dev and should be changed to libpng-dev"
        fi
...
# @fn setupDirectories()
# @brief Makes sure some required ares directories and files are created.
function setupDirectories() {
    ...
    if [[ ! -f "$config" ]]; then
        echo "# this file can be used to enable/disable ares autoconfiguration features" >"$config"
    fi
...
## @brief Creates a default retroarch.cfg for specified system in `/opt/ares/configs/$system/retroarch.cfg`.
...
# added by ARES-Setup - $res mode for emulators
mode "$res"
...
function getPlatformConfig() {
    ...
    # workaround for ARES platform
    [[ "$key" == "ares_fullname" ]] && ini_value="ARES"
    echo "$ini_value"
}
...
## Helper function to manage DKMS modules installed by ARES
