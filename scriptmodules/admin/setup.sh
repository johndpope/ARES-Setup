...
rp_module_id="setup"
rp_module_desc="GUI based setup for ARES"
rp_module_section=""
...
function rps_logStart() {
    ...
    echo "ARES-Setup version: $__version ($(git -C "$scriptdir" log -1 --pretty=format:%h))"
...
function depends_setup() {
    ...
    if [[ ! -f "$rootdir/VERSION" ]]; then
        joy2keyStop
        exec "$scriptdir/ares_packages.sh" setup post_update gui_setup
    fi

    if isPlatform "rpi" && isPlatform "mesa" && ! isPlatform "rpi4"; then
        printMsgs "dialog" "WARNING: You have the experimental desktop GL driver enabled. This is NOT supported by ARES, and Emulation Station as well as emulators may fail to launch.\n\nPlease disable the experimental desktop GL driver from the raspi-config 'Advanced Options' menu."
    fi

    if [[ "$__os_debian_ver" -eq 8 ]]; then
        printMsgs "dialog" "Raspbian/Debian Jessie and versions of Ubuntu below 16.04 are no longer supported.\n\nPlease install ARES 4.4 or newer from a fresh image which is based on Raspbian Stretch (or if running Ubuntu, upgrade your OS)."
    fi
...
function updatescript_setup()
{
    ...
    printHeading "Fetching latest version of the ARES Setup Script."
    pushd "$scriptdir" >/dev/null
    if [[ ! -d ".git" ]]; then
        printMsgs "dialog" "Cannot find directory '.git'. Please clone the ARES Setup script via 'git clone https://github.com/Retro-Arena/ARES-Setup.git'"
        popd >/dev/null
        return 1
    fi
    local error
    ...
    printMsgs "dialog" "Fetched the latest version of the ARES Setup script."
    return 0
}
...
function post_update_setup() {
    ...
    {
        rps_logStart
        # run _update_hook_id functions - eg to fix up modules for ARES-Setup 0.x install detection
    ...
    printMsgs "dialog" "NOTICE: The ARES-Setup script and pre-made ARES SD card images are available to download for free from https://odroidarena.com/\n\nThe pre-built ARES image includes software that has non commercial licences. Selling ARES images or including ARES with your commercial product is not allowed.\n\nNo copyrighted games are included with ARES.\n\nIf you have been sold this software, you can let us know about it by emailing aresproject@gmail.com."
    ...
    while true; do
        ...
        case "$choice" in
            ...
            X)
                ...
                [[ "${__mod_section[$idx]}" == "core" ]] && text+="\n\nWARNING - core packages are needed for ARES to function!"
...
function update_packages_gui_setup() {
    ...
    if [[ "$update" != "update" ]]; then
        ...
        joy2keyStop
        exec "$scriptdir/ares_packages.sh" setup post_update update_packages_gui_setup update
    fi
...
function packages_gui_setup() {
    ...
    for section in core opt lr sa prt fe driver; do
...
{
    dialog --defaultno --yesno "Are you sure you want to uninstall ARES?" 22 76 2>&1 >/dev/tty || return 0
    dialog --defaultno --yesno "Are you REALLY sure you want to uninstall ARES?\n\n$rootdir will be removed - this includes configuration files for all ARES components." 22 76 2>&1 >/dev/tty || return 0
    clear
    printHeading "Uninstalling ARES"
    for idx in "${__mod_idx[@]}"; do
        rp_isInstalled "$idx" && rp_callModule $idx remove
    done
    rm -rfv "$rootdir"
    ...
    if dialog --defaultno --yesno "Do you want to remove all the system packages that ARES depends on? \n\nWARNING: this will remove packages like SDL even if they were installed before you installed ARES - it will also remove any package configurations - such as those in /etc/samba for Samba.\n\nIf unsure choose No (selected by default)." 22 76 2>&1 >/dev/tty; then
        clear
    ...
    printMsgs "dialog" "ARES has been uninstalled."
}
...
function gui_setup() {
    ...
    while true; do
        ...
        cmd=(dialog --backtitle "$__backtitle" --title "ARES-Setup Script" --cancel-label "Exit" --item-help --help-button --default-item "$default" --menu "Version: $__version (running on $__os_desc)\nLast Commit: $commit" 22 76 16)
        options=(

            U "Update Scripts"
            "U Update the ARES-Setup script to the latest version."
        ...
        case "$choice" in
            ...
            U)
                dialog --defaultno --yesno "Are you sure you want to update the ARES-Setup script ?" 22 76 2>&1 >/dev/tty || continue
