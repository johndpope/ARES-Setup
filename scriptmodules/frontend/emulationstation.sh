...
rp_module_desc="EmulationStation - Frontend used by ARES for launching emulators"
...
_EOF_
    ...
    if isPlatform "x11"; then
        ...
        cp "$scriptdir/scriptmodules/$md_type/emulationstation/ares.svg" "/usr/local/share/icons/"
        cat > /usr/local/share/applications/ares.desktop << _EOF_
[Desktop Entry]
Type=Application
...
Comment[de_DE]=ARES
Comment=ARES
Icon=/usr/local/share/icons/ARES.svg
Categories=Game
_EOF_
    fi
}
...
function remove_emulationstation() {
    ...
    if isPlatform "x11"; then
        rm -rfv "/usr/local/share/icons/ares.svg" "/usr/local/share/applications/ares.desktop"
    fi
}
