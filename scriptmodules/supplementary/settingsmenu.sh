...
function _update_hook_settingsmenu() {
    # to show as installed when upgrading to ARES-setup 0.x
    if ! rp_isInstalled "$md_idx" && [[ -f "$configdir/all/emulationstation/gamelists/ares/gamelist.xml" ]]; then
        mkdir -p "$md_inst"
        # to stop older scripts removing when launching from settings menu in ES due to not using exec or exiting after running ARES-setup from this module
        touch "$md_inst/.ares"
    fi
}
...
NOTE: This menu is EXPERIMENTAL. Use at your own risk and be sure to backup your image!"
    ...
    setESSystem "ARES" "ares" "$rpdir" ".rp .sh" "sudo $scriptdir/ares_packages.sh settingsmenu launch %ROM% </dev/tty >/dev/tty && clear" "" "ares"
    ...
    for i in "${!files[@]}"; do
        ...
        for function in $(compgen -A function _add_rom_); do
            "$function" "ares" "ARES" "$file.rp" "$name" "$desc" "$image" "$hidden"
        done
    done
}
...
function remove_settingsmenu() {
    ...
    rm -rf "$configdir/all/emulationstation/gamelists/ares"
    delSystem ares
}
