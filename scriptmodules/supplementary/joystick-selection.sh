...
# ATTENTION! #
##############
# This file has changed (26-March-2017). Now it works as an ARES scriptmodule.
# Name this file as ~/RetroPie-Setup/scriptmodules/suplementary/joystick-selection.sh
# and then execute the ares_setup.sh script.
# To install the joystick-selection tool, go to
# Manage packages >> Manage optional packages >> joystick-selection >> Install from source
...
function sources_joystick-selection() {
    gitPullOrClone "$md_build" "https://github.com/Retro-Arena/RetroPie-joystick-selection.git"
}
...
function install_joystick-selection() {
    ...
    cp -nv "$configdir/all/emulationstation/gamelists/ares/gamelist.xml" "$gamelistxml"
