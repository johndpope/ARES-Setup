...
function createdir_usbromservice() {
    if ls -la /var/run/usbmount | grep "\->" >/dev/null; then
        ...
        printMsgs "dialog" "The 'ARES/roms' directory structure has been created on the USB drive"
    else
...
function synctousb_usbromservice() {
    if ls -la /var/run/usbmount | grep "\->" >/dev/null; then
        if [[ ! -d "/media/usb0/ARES/roms" ]]; then
            mkdir -p "/media/usb0/ARES" "/media/usb0/ARES/roms"
        fi
        echo "---------------------------------------------------"
        ...
        rsync -rtu --human-readable --no-i-r --copy-links --info=progress2 "$datadir/roms/" "/media/usb0/ARES/roms/"
...
function synctosd_usbromservice() {
    if ls -la /var/run/usbmount | grep "\->" >/dev/null; then
        echo "---------------------------------------------------"
        echo "Sync from USB to SD 'ARES' is now starting..."
        rsync -rtu --human-readable --no-i-r --copy-links --info=progress2 "/media/usb0/ARES-sync/" "$datadir/"
        printMsgs "dialog" "Sync completed!"
    else
        printMsgs "dialog" "USB drive is not mounted"
    fi
...
function gui_usbromservice() {
    ...
    while true; do
        ...
        options=(
            ...
            3 "Sync from USB 'ARES-sync' to SD 'ARES'"
        ...
        if [[ -n "$choice" ]]; then
            case "$choice" in
                1)
                    ...
                    ;;
                2)
                    printMsgs "dialog" "This will sync only the folder structure of the SD card '$datadir/roms' directory to the USB drive 'ARES/roms' directory.\n\nPress OK to continue."
                    createdir_usbromservice
                    ;;
                3)
                    printMsgs "dialog" "This will sync all folders and files from the USB drive 'ARES-sync' directory to the SD card '$datadir' directory.\n\nThe SD card has $avail of available space. Ensure the USB drive 'ARES-sync' directory does NOT have more than $avail of files being copied.\n\nWARNING: This may take a long time. Press OK to continue."
