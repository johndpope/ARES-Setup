...
function toggle_setupscript() {
    if [[ -e "$home/.config/setupscript" ]]; then
        rm -rf "$home/.config/setupscript"
        printMsgs "dialog" "Disabled ARES-Setup AutoUpdate"
    else
        touch "$home/.config/setupscript"
        printMsgs "dialog" "Enabled ARES-Setup AutoUpdate\n\nThe update will occur weekly on Sundays at 03:00 UTC."
    fi
}
...
function gui_autoupdate() {
    while true; do
        ...
        [[ -e "$home/.config/setupscript" ]] && options+=(1 "Disable ARES-Setup AutoUpdate (Weekly)") || options+=(1 "Enable RetroArena-Setup AutoUpdate")     
        [[ -e "$home/.config/retrohub" ]] && options+=(2 "Disable Retrohub AutoUpdate (Weekly)") || options+=(2 "Enable Retrohub AutoUpdate")
        
        local cmd=(dialog --backtitle "$__backtitle" --menu "AutoUpdate: an ARES Exclusive" 22 86 16)
