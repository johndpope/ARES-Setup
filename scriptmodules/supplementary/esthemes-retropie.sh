...
rp_module_id="esthemes-retropie"
rp_module_desc="Install themes for Emulation Station from RetroPie"
rp_module_section="config"

function depends_esthemes-retropie() {
...
function install_theme_esthemes-retropie() {
    local theme="$1"
    local repo="$2"
    if [[ -z "$repo" ]]; then
        repo="RetroPie"
...
function uninstall_theme_esthemes-retropie() {
    local theme="$1"
...
function gui_esthemes-retropie() {
    local themes=(
        'RetroPie carbon'
        ...
        case "$choice" in
            ...
            *)
                ...
                if [[ "${status[choice]}" == "i" ]]; then
                    ...
                    case "$choice" in
                        1)
                            rp_callModule esthemes-retropie install_theme "$theme" "$repo"
                            ;;
                        2)
                            rp_callModule esthemes-retropie uninstall_theme "$theme"
                            ;;
                    esac
                else
                    rp_callModule esthemes-retropie install_theme "$theme" "$repo"
