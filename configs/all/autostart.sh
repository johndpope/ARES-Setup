...
if [[ -e "$HOME/.config/themerandomizer" ]]; then
    ...
    ls /opt/ares/configs/all/emulationstation/themes >> /tmp/themes
    themecount=`cat /tmp/themes |wc -l`
    curtheme=`cat /opt/ares/configs/all/emulationstation/es_settings.cfg |grep ThemeSet |cut -f4 -d '"'`
    r=$(( $RANDOM % ${themecount} +1 ))
    newtheme=`sed -n "${r}p" /tmp/themes`
    perl -pi -w -e 's/<string name=\"ThemeSet\" value=\"'${curtheme}'\" \/>/<string name=\"ThemeSet\" value=\"'${newtheme}'\" \/>/g;' /opt/ares/configs/all/emulationstation/es_settings.cfg
fi
...
if [ -a $home/.config/ogst* ];  then
    mplayer -quiet -nolirc -nosound -vo fbdev2:/dev/fb1 -vf scale=320:240 "$HOME/ARES/casetheme/es.png" &> /dev/null
fi
